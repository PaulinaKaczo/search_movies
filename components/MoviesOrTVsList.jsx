              ...
              <Grid container spacing={{md: 3 }} columns={{ md: 16 }}>
                {list.map((item) => (
                    <Grid md={4} key={item.id}>
                      ...
                      <button className='favourite_btn'  onClick={selectedCategories === 'movie'
                          ? (() => handleAddToFavourites(favouritesMovie, item, setFavouritesMovie))
                          : (() => handleAddToFavourites(favouritesTV, item, setFavouritesTV))}>

                        <FontAwesomeIcon icon={faHeart} color={isLiked(item.id) ? 'red' : 'grey'}/>
                      </button>
                    </Grid>
                ))}
              </Grid>
